...
Type objective_function<Type>::operator() ()
{
  ...
  int n_obs_types = C_i.col(0).size();
  ...
  matrix<Type> E_count_obs(n_obs_types,n_i);
  ...
  for(int i=0; i<n_i; i++){
    for(int isp=0;isp<n_sp;isp++){
      ...
      for(int itype=0;itype<n_obs_types;itype++){
        E_count_obs(itype,i)+=E_count_sp(isp,i)*Psi(isp,itype);
      }
    }
    for(int itype = 0;itype<n_obs_types;itype++){
      if( !isNA(C_i(itype,i)) ) jnll_comp(0) -= dpois( C_i(itype,i), E_count_obs(itype,i), true );
    }
  }
  //std::cout<<Thin_i<<"\n\n";
  ...
  for(int isp=0;isp<n_sp;isp++){
    total_abundance(isp)=Z_s.row(isp).sum();
  }
