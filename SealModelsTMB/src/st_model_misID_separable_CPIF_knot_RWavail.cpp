...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SPARSE_MATRIX(X_rw1);  //matrix for distributing species-specific AR1 random effects to observation level 
  DATA_ARRAY(MisID_mu);
  ...
  //PARAMETER_MATRIX(thin_AR_RE);   // dim = (n_species,t_steps)
  PARAMETER_VECTOR(thin_rho_logit); // 1 for each species 
  PARAMETER_VECTOR(thin_sigma_log); 
  ...
  vector<Type> Thin_rho(n_sp);
  vector<Type> Thin_sigma(n_sp);
  Type cur_sum;
  ...
  for( int isp=0; isp<n_sp; isp++){
    ...
    Thin_rho(isp) = f(thin_rho_logit(isp));
    Thin_sigma(isp) = exp(thin_sigma_log(isp));
  }
  
  int n_tsp = n_t*n_sp;
  matrix<Type>ARcov(n_tsp,n_tsp);
  for(int isp=0;isp<n_sp;isp++){
    Type cur_var = pow(Thin_sigma(isp),Type(2.0))/(Type(1.0)-pow(Thin_rho(isp),Type(2.0)));
    for(int it=0;it<n_t;it++){
      for(int it2=0;it2<n_t;it2++){
        ARcov(isp*n_t+it,isp*n_t+it2)=pow(Thin_rho(isp),Type(abs(it-it2)))*cur_var;
      }
    }
  }
  //std::cout<<"ARcov "<<ARcov.block(0,0,4,4)<<'\n'<<'\n';
  
  int n_isp = n_i*n_sp;
  matrix<Type>Sigma_logit_thin2(n_isp,n_isp);
  Sigma_logit_thin2 = Sigma_logit_thin + X_rw1 * ARcov * X_rw1.transpose();
  //std::cout<<"Sig thin2 "<<Sigma_logit_thin2.block(0,0,4,4)<<'\n'<<'\n';
  
  
  MVNORM_t<Type>neg_log_density_thin(Sigma_logit_thin2);
  ...
  //for(int i=0;i<n_i;i++){
  //  jnll_comp(2)+=pow(thin_logit_i(i)-thin_mu_logit(i),2.0);
  //}
  //std::cout<<"Thin_rho "<<Thin_rho<<'\n';
  //std::cout<<"Thin_sigma "<<Thin_sigma<<'\n';
