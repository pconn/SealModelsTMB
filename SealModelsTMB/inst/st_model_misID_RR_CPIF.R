...
#for(i in 1:n_sim){
    ...
    # Fix parameters
    Map = list()
    #fix common spatial scale, range params
    Map[["logrange"]] = factor( rep(1,n_species) )
    Map[["logsmooth"]] = factor( rep(2,n_species) )
    Map[["phi_logit"]] = factor( rep(3,n_species) )
...
#     
    ...
    # output
    Results=list(N=Results_N,Par=Results_par)
