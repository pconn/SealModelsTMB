...
TmbFile = "C:/Users/paul.conn/git/SealModelsTMB/SealModelsTMB/src/st_model_misID"
...
#for(i in 1:n_sim){
    ...
    Etainput_st[,1:n_cells,1]=Eta_s[,,1]
    Etainput_st[,1:n_cells,2:t_steps]=Delta_s[,,2:t_steps]*8
    Beta_init = matrix(0,n_species,ncol(Data$X_s))
    Beta_init[,1] = log(colSums(Ztrue_s[1:625,])/625-0.5*sqrt(apply(Ztrue_s[1:625,],2,'var')))
    Beta_init[,2] = betax
    Params = list("Beta"=Beta_init, "logtau_eta"=rep(0,n_species), "logkappa_eta"=rep(-.9,n_species),"logtau_delta"=rep(2.5,n_species), "logkappa_delta"=rep(-.9,n_species),"Etainput_st"=Etainput_st,"thin_logit_i"=thin_logit,"MisID_pars"=MisID_pars,"phi_logit"=4.0)
