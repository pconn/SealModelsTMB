...
TmbFile = "C:/Users/paul.conn/git/SealModelsTMB/SealModelsTMB/src/spatial_model_misID"
...
for(i in 1:n_sim){
    ...
    C_i_true = matrix(0,n_species,n_samp)
    C_i = matrix(0,n_obs_types,n_samp)
    for(isp in 1:n_species){
      Cur_lambda = Ztrue_s[isp,s_i]*prop_sampled*Thin[(isp-1)*n_samp+c(1:n_samp)]
      C_i_true[isp,] = rpois(n=n_samp,lambda=Cur_lambda)
      for(isamp in 1:n_samp){
        if(C_i_true[isp,isamp]>0)C_i[,isamp]=C_i[,isamp]+rmultinom(1,C_i_true[isp,isamp],MisID_real[isp,])
      }
    }  
