...
TmbFile = "C:/Users/paul.conn/git/SealModelsTMB/SealModelsTMB/src/st_model_misID_separable_CPIF_knot_RWavail"
...
#for(isim in 1:n_sim){
  ...
  for(isp in 1:n_species){
    ...
    X_rw1 = matrix(0,n_samp,t_steps)
    for(it in 1:t_steps)X_rw1[n_samp_t*(it-1)+c(1:n_samp_t),it]=1
    X_rw1 = Matrix(kronecker(diag(n_species),X_rw1))
    Data = list( "Options_vec"=Options_vec, "C_i"=C_i, "P_i"=Prop_sampled,"A_s"=rep(1,n_cells*t_steps),"S_i"=S_i-1,"Y_s"=Y_s,"X_s"=X_s, "spde"=spde,"thin_mu_logit"=thin_logit,"Sigma_logit_thin"=Sigma_logit_thin,"X_rw1"=X_rw1,"MisID_mu"=MisID_pars,"MisID_Sigma"=MisID_Sigma,"Kmap"=Kmap)
    ...
    Beta_init[,1] = betax 
    Params = list("log_N"=log(N*exp(rnorm(n_species,0,0.1))),"Beta"=Beta_init, "logtau_eta"=rep(0,n_species), "logkappa_eta"=rep(0,n_species),"logit_rho"=rep(0,n_species),"Etainput_st"=Etainput_st,"thin_logit_i"=thin_logit,"thin_rho_logit"=rep(1,n_species),"thin_sigma_log"=rep(-2,n_species),"MisID_pars"=MisID_pars)
